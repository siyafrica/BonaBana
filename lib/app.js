...
//            max: 200, // 100 is default
	...
	var tdrive_chart = c3.generate({
		...
		axis: {
			x: {
				type: 'category',
				categories: ["Topic: Education", "Diversity: Gauteng", "Rights: Privacy", "Issues: Context indepth", "Voices of Children", "Ethics: Child Best Interest"]
			}
		},
		legend: {
			position: 'right'
	...
	var education_level = c3.generate({
		...
		axis: {
			x: {
				type: 'category',
				categories: ["Grade 0", "Grade 1", "Grade 2", "Grade 3", "Grade 4", "Grade 5", "Grade 6", "Grade 7", "Grade 8", "Grade 9", "Grade 10", "Grade 11", "Grade 12", "No schooling", "Not applicable", "Other"]
			},
		}
	});
...
$(document).ready(function(){
	//The script for Dynatable
	$.getJSON("data/gap-finder.JSON", function(data) {
    $("#gap-finder").dynatable({
        dataset: {
            records: data
        	}
    	});
